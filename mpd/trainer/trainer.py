...
def save_models_to_disk(models_prefix_l, total_steps, checkpoints_dir=None):
    for model, prefix in models_prefix_l:
        if model is not None:
            save_model_to_disk(model, total_steps, checkpoints_dir, prefix=f"{prefix}_")
            for submodule_key, submodule_value in model.submodules.items():
                save_model_to_disk(submodule_value, total_steps, checkpoints_dir, prefix=f"{prefix}_{submodule_key}_")


def save_model_to_disk(model, total_steps, checkpoints_dir=None, prefix="model_"):
    ...
    torch.save(model.state_dict(), os.path.join(checkpoints_dir, f"{prefix}_iter_{total_steps:06d}_state_dict.pth"))
    torch.save(model, os.path.join(checkpoints_dir, f"{prefix}current.pth"))
    torch.save(model, os.path.join(checkpoints_dir, f"{prefix}_iter_{total_steps:06d}.pth"))
...
def do_summary(summary_fn, train_steps_current, model, batch_dict, loss_info, datasubset, **kwargs):
    ...
    with torch.no_grad():
        ...
        summary_fn(
            train_steps_current, model, batch_dict=batch_dict, loss_info=loss_info, datasubset=datasubset, **kwargs
        )
...
def train(
    ...
    **kwargs,
    ...
    if optimizers is None:
        optimizers = [torch.optim.AdamW(lr=lr, params=model.parameters())]

    # learning rate scheduler
    scheduler = None
    if lr_scheduler:
        assert len(optimizers) == 1, "Only one optimizer is supported for now."
        iters_dataloader = len(train_dataloader)
    ...
    save_models_to_disk([(model, "model"), (ema_model, "ema_model")], 0, checkpoints_dir)

    train_steps_current = 0
    with tqdm(total=num_train_steps, mininterval=1 if debug else 60) as pbar:
        train_losses_l = []
        validation_losses_l = []
        for epoch in range(epochs):
            ...
            for step, train_batch_dict in enumerate(train_dataloader):
                t_dataloading_end = time.perf_counter()
                ####################################################################################################
                # TRAINING LOSS
                ####################################################################################################
                with TimerCUDA() as t_training_loss:
                    ...
                    train_losses_log = {"total_loss": train_loss_batch}
                    for loss_name, loss in train_losses_info.items():
                        train_losses_log[loss_name] = to_numpy(loss).mean().item()
                ...
                if train_steps_current % steps_til_summary == 0:
                    ...
                    print(f"t_dataloading      : {t_dataloading_end - t_dataloading_start:.4f} sec")
                    ...
                    if val_dataloader is not None:
                        with TimerCUDA() as t_validation_loss:
                            ...
                            val_losses_info = defaultdict(list)
                            total_val_loss = 0.0
                            for step_val, batch_dict_val in enumerate(val_dataloader):
                                ...
                                for name, value in val_loss_info.items():
                                    single_loss = to_numpy(value)
                                    val_losses_info[name].append(single_loss)
                            ...
                            validation_losses_log["VALIDATION total_loss"] = np.sum(list(validation_losses.values()))
                            for loss_name, loss in val_losses_info.items():
                                single_loss = np.mean(loss).item()
                                validation_losses_log[f"VALIDATION {loss_name}"] = single_loss
                ...
                with TimerCUDA() as t_training_optimization:
                    ...
                    if scheduler:
                        scheduler.step(epoch + step / iters_dataloader)
                ...
                if (steps_til_checkpoint is not None) and (train_steps_current % steps_til_checkpoint == 0):
                    save_models_to_disk(
                        [(model, "model"), (ema_model, "ema_model")], train_steps_current, checkpoints_dir
                    )
                ...
                if train_steps_current >= num_train_steps:
                    break
                t_dataloading_start = time.perf_counter()
        ...
        save_models_to_disk([(model, "model"), (ema_model, "ema_model")], train_steps_current, checkpoints_dir)
        save_losses_to_disk(train_losses_l, validation_losses_l, checkpoints_dir)
