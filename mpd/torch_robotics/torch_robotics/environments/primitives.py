...
class PrimitiveShapeField(ABC):
    ...
    def __init__(self, dim=3, tensor_args=DEFAULT_TENSOR_ARGS):
        self.dim = dim
...
def plot_sphere(ax, center, pos, radius, cmap):
    ...
    z = radius * np.cos(v)
    ax.plot_surface(x + center[0] + pos[0], y + center[1] + pos[1], z + center[2] + pos[2], cmap=cmap, alpha=1)


class MultiSphereField(PrimitiveShapeField):

    def __init__(self, centers, radii, tensor_args=DEFAULT_TENSOR_ARGS):
        """
        Parameters
        ----------
            centers : numpy array
...
class MultiBoxField(PrimitiveShapeField):
    ...
    def __init__(self, centers, sizes, tensor_args=DEFAULT_TENSOR_ARGS):
        """
        Parameters
        ----------
            centers : numpy array
    ...
    def add_to_occupancy_map(self, obst_map):
        for center, size in zip(self.centers, self.sizes):
            ...
            if n_dim == 2:
                obst_map.map[
                    c_y - ceil(h / 2.0) + obst_map_origin_yi : c_y + ceil(h / 2.0) + obst_map_origin_yi,
                    c_x - ceil(w / 2.0) + obst_map_origin_xi : c_x + ceil(w / 2.0) + obst_map_origin_xi,
                ] += 1
            else:
                depth = size[2]
                c_z = ceil(center[2] / obst_map.cell_size)
                ...
                obst_map.map[
                    c_y - ceil(h / 2.0) + obst_map_origin_yi : c_y + ceil(h / 2.0) + obst_map_origin_yi,
                    c_x - ceil(w / 2.0) + obst_map_origin_xi : c_x + ceil(w / 2.0) + obst_map_origin_xi,
                    c_z - ceil(d / 2.0) + obst_map_origin_zi : c_z + ceil(d / 2.0) + obst_map_origin_zi,
                ] += 1
        return obst_map
    ...
    def draw_box(self, ax, i, point, a, b, rot, trans, color="gray"):
        rectangle = plt.Rectangle((point[0], point[1]), a, b, color=color, linewidth=0, alpha=1)
...
class MultiRoundedBoxField(MultiBoxField):
    ...
    def draw_box(self, ax, i, point, a, b, rot, trans, color="gray"):
        rounded_box = FancyBboxPatch(
            ...
            b,
            color=color,
            boxstyle=BoxStyle.Round(pad=0.0, rounding_size=to_numpy(self.radius[i]).item()),
        )
...
#
#     def __init__(self, centers, radii, heights, tensor_args=DEFAULT_TENSOR_ARGS):
#         """
#         Parameters
#         ----------
#             centers : numpy array
...
class ObjectField(PrimitiveShapeField):
    ...
    def __init__(self, primitive_fields, name="object", pos=None, ori=None, reference_frame="world", **kwargs):
        """
    ...
    def compute_signed_distance_impl(self, x, get_gradient=False):
        ...
        if get_gradient:
            ...
            sdf, sdf_idx = torch.min(torch.stack(sdf_fields, dim=-1), dim=-1)
            grad_sdf_fields_stacked = torch.stack(grad_sdf_fields, dim=-1)
            # https://stackoverflow.com/a/67951672/4585592
            grad_sdf = grad_sdf_fields_stacked[torch.arange(grad_sdf_fields_stacked.size(0)), :, sdf_idx]
            return sdf, grad_sdf
        else:
            sdf_fields = []
            for field in self.fields:
...
if __name__ == "__main__":
    ...
    obj_field.set_position_orientation(pos=np.array([-0.5, 0.0, 0.0]), ori=ori)
    ...
    ax.quiver(to_numpy(X_flat), to_numpy(Y_flat), grad_sdf_np[:, 0], grad_sdf_np[:, 1], color="red")
