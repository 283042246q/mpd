...
def _position_camera(camera: Camera, sim, env_name: str, camera_eye=None, camera_target=None):
    ...
    elif env_name == "EnvOpenDrawerShelf":
        # The drawer and shelf open towards -x. Keep the camera predominantly
        # on that side so neither cabinet back nor side wall occludes the task.
        eye_values = [-3.0, -1.2, 1.6]
        target_values = [0.55, -0.05, 0.45]
    elif env_name == "EnvThreePillarsPassage":
        eye_values = [-2.4, -2.0, 1.65]
        target_values = [0.40, 0.0, 0.55]
