...
10 条真实 GPU 冒烟和一次 100 条共享 GPU 负载测试已经成功，actor restart 均为 0，
输出通过训练入口的 hash、模型资产、形状、样条和 EE context 校验。100 条证明了当前
actor/checkpoint 链路的短期稳定性，但还不能替代独占 GPU A/B 和 1,000 条 soak。原来的
...
### 100 条：dual=8、left/right=16、3 endpoint actors
...
10 个 checkpoint 的中位数为 92.27 s，p90 为 263.73 s，范围 76.70–303.23 s。长尾来自
...
3. 从新写入的阶段 wall 统计判断瓶颈：如果 endpoint actors 占主导，比较 2/3/4 个 producer
